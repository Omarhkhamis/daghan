...
import { normalizeLocale, normalizeSite, SUPPORTED_LOCALES } from "./sites";
  ...
  for (const def of definitions) {
    ...
    if (!found) {
      await prisma.section.upsert({
        where: {
          key_locale_site: { key: def.key, locale: lang, site: siteId }
        },
        update: {},
        create: {
          key: def.key,
    ...
    if (!found.data || typeof found.data !== "object") {
      updateData.data = getSectionDefaults(def.key, lang, siteId);
    }
...
export const ensureSections = async (site = DEFAULT_SITE, locale = "en") => {
  const siteId = normalizeSite(site) || DEFAULT_SITE;
  const lang = normalizeLocale(locale);
  const localesToEnsure = new Set([lang, ...SUPPORTED_LOCALES]);
